# Specification: P/Invoke Integration with NeonVM

## 1. Scope
This document specifies the methodology and architecture for integrating unmanaged code (C++ native libraries) with .NET assemblies protected by NeonVM (KoiVM custom). Since C++/CLI (Managed C++) is strictly limited to Windows environments, this specification focuses on the cross-platform P/Invoke (Platform Invocation Services) approach for macOS and Linux systems.
...
* NeonVM Virtualization Engine (KoiVM ConfuserEx Plugin).

## 3. System Architecture
NeonVM operates exclusively on Common Intermediate Language (CIL) instructions. Consequently, native code and direct P/Invoke method definitions (`[DllImport]`) cannot be virtualized because they lack managed method bodies. 
...
4. **Execution**: At runtime, NeonVM executes the wrapper logic, dynamically resolving and invoking the unmanaged functions while obscuring the surrounding managed logic (e.g., argument preparation, return value manipulation, and control flow).
...
public class NativeBridge {
    ...
    public static int Compute(int a, int b) {
        // Logic inside this method is converted to NeonVM bytecode
...
2. Compile the managed assembly:
   ...
   ```
3. Apply NeonVM protection via ConfuserEx CLI:
   ```bash
...
While NeonVM effectively hides the execution flow and data manipulation within the wrapper layer, the names of the imported native functions and their signatures remain visible in the assembly's metadata. To maximize security:
