# Specification: NeonVM Protection Methodologies

## 1. Scope
This document specifies the protection mechanisms and obfuscation methodologies implemented by NeonVM (a custom KoiVM variant) acting as a ConfuserEx plugin. The system is designed to secure .NET assemblies against reverse engineering, decompilation, and unauthorized modification through a combination of IL virtualization, environmental checks, and metadata obfuscation.

## 2. Normative References
* ECMA-335: Common Language Infrastructure (CLI) Standard
* NeonVM Virtual Machine Architecture
* ConfuserEx Core Obfuscation Pipeline

## 3. Core Virtualization Engine (NeonVM)
...
4. **Bytecode Emission**: The IR is compiled into NeonVM bytecode. This bytecode is stored within an injected `#NeonVM` metadata stream, distinct from standard `#~` or `#Strings` streams.
...
NeonVM integrates deeply with the ConfuserEx pipeline to inject active environmental defenses.
...
NeonVM explicitly neutralizes automated devirtualizers (such as OldRod). Because the encryption keys, opcode mappings, and VM structures are polymorphic per compilation, analysts must build a custom devirtualization mapping for every individual protected payload.